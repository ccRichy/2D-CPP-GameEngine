...
draw_bmp(Game_Pointers game_pointers, BMP_File* bmp, Vec2 pos, Vec2 scale)
{
    ...
    Game_Render_Buffer* render = game_pointers.render;
    float32 window_scale = game_pointers.settings->window_scale;
    
    pos.y += bmp->height;

    int32 x_start = round_i32(pos.x * window_scale);
    int32 x_end   = round_i32(x_start + (bmp->width * window_scale));
    int32 y_start = round_i32(pos.y * window_scale);
    int32 y_end   = round_i32(y_start - (bmp->height * window_scale));
    ...
    for (int Y = y_end; Y < y_start; ++Y)
    {
        ...
        for (int X = x_start; X < x_end; ++X)
        {
            ...
            if (sign(scale.x) == 1)
                 bmp_x = (int32)(((X - pos.x * window_scale)) / window_scale);
            else bmp_x = bmp->width-1 - (int32)(((X - pos.x * window_scale)) / window_scale);
                
            if (sign(scale.y) == 1)
                 bmp_y = (int32)((y_start - 1 - Y + y_offscreen_amt) / window_scale);
            else bmp_y = bmp->height - (int32)((y_start - 1 - Y + y_offscreen_amt) / window_scale);
...
void
draw_bmp_part(Game_Pointers game_pointers, BMP_File* bmp, Vec2 pos, Vec2 scale, int32 bmp_drawx, int32 bmp_drawy, int32 bmp_draw_width, int32 bmp_draw_height)
{
    //NOTE: this code was partially generated by ai
    Game_Render_Buffer* render = game_pointers.render;
    float32 window_scale = game_pointers.settings->window_scale;
    ...
    pos.y += bmp->height;
    if (sign(scale.x) == -1){
        int32 x_sub = (int32)((bmp->width - bmp_draw_width) * window_scale);
        pos.x -= bmp->width - bmp_draw_width;
    }

    int32 x_left = round_i32(pos.x * window_scale);
    int32 x_right   = round_i32(x_left + (bmp->width * window_scale));
    int32 y_top = round_i32(pos.y * window_scale);
    int32 y_bottom   = round_i32(y_top - (bmp->height * window_scale));
    ...
    for (int32 Y = y_bottom; Y < y_top; ++Y)
    {
        ...
        {            
            if (sign(scale.x) == 1)
                 bmp_x = bmp_drawx + int32((X - (pos.x * window_scale)) / window_scale);
            else bmp_x = bmp->width-1 + bmp_drawx - int32(((X) - (pos.x * window_scale)) / window_scale);
                
            if (sign(scale.y) == 1)
                 bmp_y = bmp_drawy + int32((y_top - 1 - Y + y_offscreen_amt) / window_scale);
            else bmp_y = bmp_drawy + bmp->height - int32((y_top - 1 - Y + y_offscreen_amt) / window_scale);
            ...
            if (alpha > 0.0f)
            {
                ...
                int32 drawx = X;
                // if (sign(scale.x) == -1) drawx = X - (int32)((bmp->width - bmp_draw_width) * window_scale);
