...
//helper
inline float32
game_get_scale(Game_Pointers game_pointers)
{
    return game_pointers.settings->window_scale * game_pointers.settings->zoom_scale;
}
...
draw_pixel(Game_Pointers game_pointers, Vec2 pos, Color color)
{
    ...
    float32 scale = game_get_scale(game_pointers);
...
{
    Game_Render_Buffer* render = game_pointers.render;
    // float32 win_scale = game_pointers.settings->window_scale;
    // float32 zoom_scale = game_pointers.settings->zoom_scale;
    // float32 scale = win_scale * zoom_scale;
    float32 scale = game_get_scale(game_pointers);
...
draw_bmp(Game_Pointers game_pointers, BMP_File* bmp, Vec2 pos, Vec2 scale)
{
    ...
    Game_Render_Buffer* render = game_pointers.render;
    // float32 window_scale = game_pointers.settings->draw_scale;
    float32 draw_scale = game_get_scale(game_pointers);
    float32 xscale_final = abs_f32(draw_scale * scale.x);
    float32 yscale_final = abs_f32(draw_scale * scale.y);

    // if (scale.x < 0)
    //     pos.x += bmp->width - (bmp->width * scale.x);
...
draw_bmp_part(Game_Pointers game_pointers, BMP_File* bmp, Vec2 pos, Vec2 scale, int32 bmp_drawx, int32 bmp_drawy, int32 bmp_draw_width, int32 bmp_draw_height)
{
    ...
    int32 x_right   = round_i32(x_left + (bmp->width * draw_scale));
    int32 y_top = round_i32(pos.y * draw_scale);
    int32 y_bottom   = round_i32(y_top - (bmp->height * draw_scale));
    ...
    for (int32 Y = y_bottom; Y < y_top; ++Y)
    {
        ...
        {            
            if (sign(scale.x) == 1)
                 bmp_x = bmp_drawx + int32((X - (pos.x * draw_scale)) / draw_scale);
            else bmp_x = bmp->width-1 + bmp_drawx - int32(((X) - (pos.x * draw_scale)) / draw_scale);
                
            if (sign(scale.y) == 1)
                 bmp_y = bmp_drawy + int32((y_top - 1 - Y + y_offscreen_amt) / draw_scale);
            else bmp_y = bmp_drawy + bmp->height - int32((y_top - 1 - Y + y_offscreen_amt) / draw_scale);
