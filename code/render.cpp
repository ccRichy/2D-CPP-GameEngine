...
//     f32 xscale = abs_f32(draw_scale * scale.x);
//     f32 yscale = abs_f32(draw_scale * scale.y);
//     pos += game_get_draw_pos();
//     pos.y += bmp->height * abs_f32(scale.y);
    
//     i32 x_left   = round_i32(pos.x * draw_scale);
//     i32 x_right  = round_i32(x_left + (bmp->width * xscale));
//     i32 y_bottom = round_i32(pos.y * draw_scale);
//     i32 y_top   =  round_i32(y_bottom - (bmp->height * yscale));
//     i32 y_offscreen_amt = 0;
...
//     u32* buffer_pixel_to_draw_into = (u32*)render->memory;
//     u8* buffer_pixel_y = (u8*)buffer_pixel_to_draw_into + y_top;
//     for (int Y = y_top; Y < y_bottom; ++Y){
//         buffer_pixel_to_draw_into = (u32*)(buffer_pixel_y + x_left);
//         // f32 xpixel_increment = xscale;
//         // f32 xpixel_check = 0;
//         for (int X = x_left; X < x_right; ++X){
//             // i32 bmpx = ;
//             // i32 bmpy = ;
//             i32 bmp_pixel_index = ((bmp->height - (i32)(Y / yscale)-2) * bmp->width) + (i32)((X + x_right) / xscale);
//             u32 col = bmp_pixels[bmp_pixel_index];

//             *buffer_pixel_to_draw_into++ = col;                
//             // xpixel_check += xpixel_increment;
//         }
//         buffer_pixel_y += render->pitch;
//     }
// }
