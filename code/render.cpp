...
draw_rect(Vec2f pos, Vec2f size, Color color)
{
    ...
    //real draw pos (on render buffer)
    int32 x_left =   round_i32(pos.x * scale);
    int32 x_right =  ceil_i32(x_left + (size.x * scale));
    int32 y_top =    round_i32(pos.y * scale);
    int32 y_bottom = ceil_i32(y_top + (size.y * scale));
...
draw_bmp(BMP_Data* bmp, Vec2f pos, Vec2f scale)
{
    ...
    int32 y_bottom = round_i32(pos.y * draw_scale);
    int32 y_top   = round_i32(y_bottom - (bmp->height * yscale_final));
    int32 y_offscreen_amt = 0;
    
    if (x_left < 0)
        x_left = 0;
    if (x_right > render->width)
        x_right = render->width;
    if (y_top < 0)
        y_top = 0;
    if (y_bottom > render->height){
        y_offscreen_amt = y_bottom - render->height;
        y_bottom = render->height;
    }
    ...
    for (int Y = y_top; Y < y_bottom; ++Y)
    {
        ...
        for (int X = x_left; X < x_right; ++X)
        {
            ...
            // bmp_y = (int32)((y_bottom - 1 - Y + y_offscreen_amt) / yscale_final);
            if (sign(scale.x) == 1)
                bmp_x = (int32)(((X - pos.x * draw_scale)) / xscale_final);
            else 
                bmp_x = bmp->width-1 - (int32)(((X - pos.x * draw_scale)) / xscale_final);
                
            if (sign(scale.y) == 1)
                bmp_y = (int32)((y_bottom - 1 - Y + y_offscreen_amt) / yscale_final);
            else
                bmp_y = bmp->height - (int32)((y_bottom - 1 - Y + y_offscreen_amt) / yscale_final);
            ...
            // color_new = color_get_transparent( (Color)color_prev, color_new );

            buffer_pixel[X] = color_new;
                // }                
            // }
        }
    }
...
//this function is very inefficient and needs to be rewritten
void
draw_bmp_part_test(BMP_Data* bmp, Vec2f pos, Vec2f scale_overall, V2i bmp_draw_pos, V2i bmp_draw_size)
{
    Assert(bmp_draw_size.x <= bmp->width && bmp_draw_size.y <= bmp->height);
    
    //NOTE: this code was partially generated by ai
    Game_Render_Buffer* render = pointers->render;
    f32 draw_scale   = game_get_draw_scale();
    f32 xscale_final = abs_f32(draw_scale * scale_overall.x);
    f32 yscale_final = abs_f32(draw_scale * scale_overall.y);
    i32 xscale_dir   = sign(scale_overall.x);
    i32 yscale_dir   = sign(scale_overall.y);
    pos             += game_get_draw_pos();

    pos.y += (bmp_draw_size.y - bmp_draw_pos.y) * abs_f32(scale_overall.y);

    i32 x_left   = round_i32(pos.x * draw_scale);
    i32 x_right  = round_i32(x_left + abs_f32((bmp_draw_size.x) * xscale_final));
    i32 y_bottom = round_i32(pos.y * draw_scale);
    i32 y_top    = round_i32(y_bottom - abs_f32((bmp_draw_size.y - bmp_draw_pos.y) * yscale_final));
    i32 x_offscreen_left = 0;
    i32 y_offscreen_top = 0;
    
    if (x_left < 0){
        x_offscreen_left = abs_i32(x_left);
        x_left = 0;
    }
    if (x_right > render->width)
        x_right = render->width;
    //TODO: variable names are wrong here and certain things are even pointless
    if (y_top < 0){
        y_offscreen_top = abs_i32(y_top);
        y_top = 0;
    }
    if (y_bottom > render->height)
        y_bottom = render->height;

    u32* bmp_pixels = bmp->pixels;
    i32 bmp_x = bmp_draw_pos.x;
    i32 bmp_y = bmp_draw_pos.y;
    
    for (i32 Y = y_top; Y < y_bottom; ++Y){
        u32* buffer_pixel = (u32*)((u8*)render->memory + (Y * render->pitch));
        
        for (i32 X = x_left; X < x_right; ++X){
            if (xscale_dir == 1)
                bmp_x = (bmp_draw_pos.x + (i32)((X - x_left + x_offscreen_left) / xscale_final));
            else if (xscale_dir == -1)
                bmp_x = bmp->width - 1 - (bmp_draw_pos.x + (i32)((X - x_left + x_offscreen_left) / xscale_final));

            if (yscale_dir == 1)
                bmp_y = bmp->height - 1 - (i32)((Y - y_top + y_offscreen_top) / yscale_final) - bmp_draw_pos.y;
            else if (yscale_dir == -1)
                bmp_y = (i32)((Y - y_top + y_offscreen_top) / yscale_final) - bmp_draw_pos.y;

            Color color_prev = *(Color*)&buffer_pixel[X];
            Color color_new = *(Color*)&bmp->pixels[(bmp_y * bmp->width) + bmp_x];
            u32 alpha = (color_new.a);
            //alpha
            if (color_new.a){
                u32 color_final = color_blend_alpha(color_prev.decimal, color_new.decimal, alpha);
                buffer_pixel[X] = color_final;
            }
        }
    }
}
...
{
    draw_bmp_part_test(&sprite->bmp, pos - sprite->origin, scale_overall, img_drawpos, img_drawsize);
}
inline void
...
draw_bmp_pixels(BMP_Data* bmp, Vec2f pos)
{
    ...
    i32 loop_count = bmp->width * bmp->height;
    for (i32 pixel_index = 0; pixel_index < loop_count; ++pixel_index)
    {
        ...
        draw_pixel({draw_x, pos.y}, color_cast);
